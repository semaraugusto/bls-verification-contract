...
fn main() {
    ...
    println!(
        "privkey1: {}, pubkey1: {}",
        hex::encode(to_bytes!(sk1.as_ref()).unwrap()),
        hex::encode(to_bytes!(sk1.to_public().as_ref()).unwrap())
    );
    // let sk2 = PrivateKey::generate(rng);
    // // println!("sk2: {}", hex::encode(to_bytes!(sk2.as_ref()).unwrap()));
    // println!(
    //     "privkey2: {}, pubkey2: {}",
    //     hex::encode(to_bytes!(sk2.as_ref()).unwrap()),
    //     hex::encode(to_bytes!(sk2.to_public().as_ref()).unwrap())
    // );
    // let sk3 = PrivateKey::generate(rng);
    // println!("sk3: {}", hex::encode(to_bytes!(sk3.as_ref()).unwrap()));
    // println!(
    //     "privkey3: {}, pubkey3: {}",
    //     hex::encode(to_bytes!(sk3.as_ref()).unwrap()),
    //     hex::encode(to_bytes!(sk3.to_public().as_ref()).unwrap())
    // );
    ...
    println!("asig: {}", hex::encode(to_bytes!(sig1.as_ref()).unwrap()));
    apk.verify(&message.as_bytes(), &[], &sig1, try_and_increment)
        .unwrap();
