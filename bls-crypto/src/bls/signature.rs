...
use ark_bls12_381::{Bls12_381, Fq12, G1Affine, G1Projective, G2Affine};
...
impl Signature {
    ...
    pub fn batch_verify_hashes<P: Borrow<PublicKey>>(
        ...
        let pairing = Bls12_381::product_of_pairings(&els);
        if pairing == Fq12::one() {
            Ok(())
        } else {
...
mod tests {
    ...
    use ark_bls12_381::{Bls12_381, G1Projective, G2Projective, Parameters};
    ...
    fn batch_verify_hashes() {
        ...
        let (secret_keys, public_keys_batches) = keygen_batch::<Bls12_381>(batch_size, num_keys);
        ...
        let asigs = sign_batch::<Bls12_381>(&secret_keys, &messages);
