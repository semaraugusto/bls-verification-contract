...
compile:
	...
	@cat build/Verifier.abi >> $(VERIFIER_JSON)
	@/bin/echo -n ', "bytecode": "0x' >> $(VERIFIER_JSON)
	@cat build/Verifier.bin >> $(VERIFIER_JSON)
...
# compile: clean
# 	@solc --metadata-literal --optimize --optimize-runs 5000000 --bin --abi --combined-json=abi,bin,bin-runtime,srcmap,srcmap-runtime,ast,metadata,storage-layout --overwrite -o build contracts/verifier.sol contracts/libs/math.sol contracts/libs/FpLib.sol contracts/tests/FpLibTest.sol
# 	@/bin/echo -n '{"abi": ' > $(VERIFIER_JSON)
# 	@cat build/Verifier.abi >> $(VERIFIER_JSON)
# 	@/bin/echo -n ', "bytecode": "0x' >> $(VERIFIER_JSON)
# 	@cat build/Verifier.bin >> $(VERIFIER_JSON)
