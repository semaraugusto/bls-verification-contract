...
import * as blslib from "bls-eth-wasm";
...
// const bls = require("./node_modules/bls-eth-wasm/src/index.js");
...
describe("BLS", async () => {
    ...
    before(async function() {
        ...
        await bls.deployed();
        await mcl.init();
        await blslib.init(blslib.BLS12_381);
    });
    ...
    it("test1", async function() {
        ...
        console.log("sig: ", aggSignature)
        console.log("pubkeys: ", pubkeys)
        console.log("messages: ", messages)
       
        const result = await bls.Test1(
            aggSignature,
        );
        assert.isTrue(result, `call failed`);
    });
    ...
    it("verify single signature", async function() {
        ...
        assert.isTrue(callSuccess, `call failed`);
        assert.isTrue(checkResult, `check failed`);
    });
